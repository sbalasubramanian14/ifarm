      ...
      <div class="page-content-wrapper ">
		...
		<div class="content">
			...
			if(isset($_GET['aksi']) == 'delete'){
				$nik = $_GET['nik'];
				$cek = pg_query($db, "SELECT * FROM ifarm.workers WHERE empid='$nik' AND uid='$uid'");
				if(pg_num_rows($cek) == 0){
					echo '<div class="alert alert-info alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> No data to delete.</div>';
				}else{
					$delete = pg_query($db, "DELETE FROM ifarm.workers WHERE empid='$nik' AND uid='$uid'");
					if($delete){
						echo '<div class="alert alert-primary alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> Data deleted! </div>';
					}else{
						echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> Error in deleting! .</div>';
					}
				}
			}
			?>
			...
			<form class="form-inline" method="get">
				<div class="form-group">
					<select name="filter" class="form-control" onchange="form.submit()">
						...
						<option value="Operator" <?php if($filter == 'Operator'){ echo 'selected'; } ?>>Operator</option>
						<option value="Labour" <?php if($filter == 'Labour'){ echo 'selected'; } ?>>Labour</option>
						<option value="All" <?php if($filter == 'All'){ echo 'selected'; } ?>>All</option>
					</select>
				</div>
				&nbsp;&nbsp;&nbsp;&nbsp;
				<a href="workers.php" class="btn btn-sm btn-info">Employee List</a>
				&nbsp;&nbsp;&nbsp;&nbsp;
				...
				if(pg_num_rows($sql) == 0){
					...
					while($row = pg_fetch_assoc($sql)){
						...
						<tr>
							<td>'.$no.'</td>
							<td>'.$row['empid'].'</td>
							<td><a href="workersprofile.php?empid='.$row['empid'].'"><span class="glyphicon glyphicon-user" aria-hidden="true"></span> '.$row['name'].'</a></td>
                            <td>'.$row['address'].'</td>
                            <td>'.$row['mobile'].'</td>
							<td>';
							if($row['category'] == 'Labour'){
								echo '<span class="label label-success">Labour</span>';
							}
                            else if ($row['category'] == 'Operator' ){
						...
						echo '
							...
							<td>
								...
								<a href="workersedit.php?empid='.$row['empid'].'" title="Edit Data" class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
								<a href="workers.php?aksi=delete&nik='.$row['empid'].'" title="Delete Data" onclick="return confirm(\'Are you sure? Want to delete '.$row['nama'].'?\')" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a>
							</td>
						</tr>
						';
						$no++;
